document.addEventListener("turbolinks:load", function initializeMap() {
  if ($('.admin-nht-invoice-show').length && $('.td-admin-map-invoice').length) {
    ...
    mapAdminShowInvoice = new google.maps.Map(document.getElementById('map-show-invoice'), {
    ...
    for (i = 0; i < locations_invoice.length; i++) {
      marker = addMarkerWithTimeout(mapAdminShowInvoice, locations_invoice[i]);
    ...
    mapAdminShowInvoice.fitBounds(bounds);
    ...
    calcRoute(mapAdminShowInvoice, markersAdminShowInvoice, directions);
  }
});
